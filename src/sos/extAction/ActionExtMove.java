...
public class ActionExtMove extends ExtAction {
    ...
    public ExtAction calc() {
        ...
        List<EntityID> safePath = new ArrayList<>();

        if (path != null && path.size() > 0) {

            int lastValidRoadIndex = path.size() - 1;
            for (int i = path.size() - 1; i >=0; i --){
                StandardEntity standardEntity = worldInfo.getEntity(path.get(i));
                if (standardEntity instanceof Road && !((Road)standardEntity).isBlockadesDefined()){
                    lastValidRoadIndex = i;
                }
            }

            for (int i = path.size() - 1; i >= lastValidRoadIndex; i --){
                path.remove(i);
            }
