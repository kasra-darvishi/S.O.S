...
public class SOSPathPlanning extends PathPlanning {
    ...
    public SOSPathPlanning(AgentInfo ai, WorldInfo wi, ScenarioInfo si, ModuleManager moduleManager, DevelopData developData) {
...
//        lengthWeight = new int[numberOfNodes+1][numberOfNodes+1];
        ...
        roadIsBlocked = new boolean[numberOfNodes+1][numberOfNodes+1];
        lastUpdateTime = new short[numberOfNodes+1][numberOfNodes+1];
...
//        EntityID firstTarget = targets.toArray(new EntityID[targets.size()])[0];
    ...
    private void updateWeightGraph() {
        ...
        //TODO
        //cleared and change blockades must be considered
//        heavyBlockadesMap.clear();
        ...
        for (EntityID entityID: worldInfo.getChanged().getChangedEntities()){
            ...
            if (entity instanceof Blockade){
                ...
                List<Edge> edges = blockadePosition.getEdges();
...
//                                logger.debug("a passableEdge between : " + road.getID().getValue() +" & "+ edge.getNeighbour().getValue() +" min dist 1: " + minDist1 + " mindist2 "+ minDist2);
                            ...
                            if (minDist1 < 1190 && minDist2 < 1190){
                                ...
                                roadIsBlocked[index1][index2] = true;
                                roadIsBlocked[index2][index1] = true;
                                lastUpdateTime[index1][index2] = 0;
                                lastUpdateTime[index2][index1] = 0;
                                break;
                            }
                        }
                ...
                int pointer = 0;
                while (pointer < numberOfEdges){
                    Edge edge1 = edges.get(pointer);
                    if (!edge1.isPassable()){
                        Edge edge2 = edges.get((pointer+1)%numberOfEdges);
                        if (edge2.isPassable()){
                            Edge edge3 = edges.get((pointer+2)%numberOfEdges);
                            if (!edge3.isPassable()){
                                //oh! seems like we've found the desired pattern!!
                                if (!isNormal(blockadePosition)) {
                                    checkBlockedBeing(edge1, edge2, edge3, null, arrayList, blockadePosition.getID(), false);;
                                }else{
                                    checkBlockedBeing(edge1, edge2, edge3, edges.get((pointer + 3) % numberOfEdges), arrayList, blockadePosition.getID(), true);
                                    break;
                                }
                            }
                        }
...
//            }
    ...
    /**
     *checking block being between two nonPassable edges
     */
    private void checkBlockedBeing(Edge edge1, Edge edge2, Edge edge3, Edge edge4, ArrayList<HeavyBlockade> arrayList, EntityID road, boolean isNormal){

        Line2D lineOfEdge1 = new Line2D.Double();
        Line2D lineOfEdge2 = new Line2D.Double();

        lineOfEdge1.setLine(edge1.getStart().getX(), edge1.getStart().getY(), edge1.getEnd().getX(), edge1.getEnd().getY());
        lineOfEdge2.setLine(edge3.getStart().getX(), edge3.getStart().getY(), edge3.getEnd().getX(), edge3.getEnd().getY());
...
//                                                System.out.println("36008 isBesideLine(lineOfEdge2)");
                    logger.debug("passable edge blocked between two non-passable edges : " + road.getValue() + " & " + edge2.getNeighbour().getValue());
                    int index1 = roadIndex.get(road).intValue();
                    int index2 = roadIndex.get(edge2.getNeighbour()).intValue();
                    roadIsBlocked[index1][index2] = true;
                    roadIsBlocked[index2][index1] = true;
                    lastUpdateTime[index1][index2] = 0;
                    lastUpdateTime[index2][index1] = 0;
                    if (isNormal){
                        logger.debug("and road was normal... " + road.getValue() + " & " + edge4.getNeighbour().getValue());
                        index2 = roadIndex.get(edge4.getNeighbour()).intValue();
                        roadIsBlocked[index1][index2] = true;
                        roadIsBlocked[index2][index1] = true;
                        lastUpdateTime[index1][index2] = 0;
                        lastUpdateTime[index2][index1] = 0;
...
//                                            System.out.println("36008 heavyBlockade.isBesideHeavyBlockade(heavyBlockade2)");
                            logger.debug("passable edge blocked between two non-passable edges by two HeavyBlockades: " + road.getValue() + " & " + edge2.getNeighbour().getValue());
